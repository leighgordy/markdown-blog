...
describe("Test copy-folder-contents.ts", async () => {
  ...
  test("Ensure folder contents are copied correctly for files", async () => {
    ...
    statMock.mock.mockImplementation(() => Promise.resolve({
      isDirectory: () => false,
    }));
    ...
    assert.deepStrictEqual(copyFileMock.mock.calls[0].arguments, ["src/file1.txt", "dest/file1.txt"]);
    assert.deepStrictEqual(copyFileMock.mock.calls[1].arguments, ["src/file2.txt", "dest/file2.txt"]);
  });

  test("Ensure subdirectories are copied recursively", async () => {
    readdirMock.mock.mockImplementationOnce(() => Promise.resolve(["subdir"]), 0);
    readdirMock.mock.mockImplementationOnce(() => Promise.resolve(["file3.txt"]), 1);
    statMock.mock.mockImplementationOnce(() => Promise.resolve({
      isDirectory: () => true,
    }),0);
    statMock.mock.mockImplementationOnce(() => Promise.resolve({
      isDirectory: () => false,
    ...
    assert.deepStrictEqual(copyFileMock.mock.calls[0].arguments, ["src/subdir/file3.txt", "dest/subdir/file3.txt"]);
  });
});